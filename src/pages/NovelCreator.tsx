...
import { Upload, X, BookOpen, Sparkles, Image as ImageIcon, Eye, ArrowLeft, Edit, FileText, Link as LinkIcon } from 'lucide-react';
...
const GENRE_OPTIONS = [
  ...
  'Xianxia', 'Wuxia', 'Xuanhuan', 'Cultivo', 'Sistema'
];
...
const NovelCreator = () => {
  ...
  const [formData, setFormData] = useState({
    title: '',
    cover: '',
    type: 'Novel' as 'Novel' | 'Light Novel' | 'Web Novel' | 'Fanfic',
    rating: 0,
    ...
    if (isEditMode && titles) {
      ...
      if (existingTitle) {
        setFormData({
          ...
          type: existingTitle.type as 'Novel' | 'Light Novel' | 'Web Novel' | 'Fanfic',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && editId) {
        await updateTitle.mutateAsync({ id: editId, ...dataToSubmit });
        toast({
          title: 'Novel atualizada!',
          description: 'Suas alterações foram salvas.',
        });
      } else {
        await createTitle.mutateAsync(dataToSubmit);
        toast({
          title: 'Novel criada!',
          description: 'Sua novel foi adicionada com sucesso.',
        });
      }
      navigate('/admin');
    } catch (error: any) {
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            <div className="p-3 rounded-xl bg-purple-500/10">
              {isEditMode ? <Edit className="h-6 w-6 text-purple-500" /> : <FileText className="h-6 w-6 text-purple-500" />}
            </div>
            <div>
              <h1 className="font-display text-3xl font-bold">
                {isEditMode ? 'Editar Novel' : 'Criar Nova Novel'}
              </h1>
              <p className="text-muted-foreground">
                {isEditMode ? 'Atualize os dados da novel' : 'Adicione uma nova novel de texto'}
              </p>
            </div>
          </div>
        </div>
                  ...
                  <div className="space-y-4">
                    {formData.cover ? (
                      ...
                      <label 
                        ...
                        className="flex flex-col items-center justify-center w-full aspect-[2/3] border-2 border-dashed border-border rounded-lg cursor-pointer hover:border-purple-500/50 hover:bg-purple-500/5 transition-colors"
                      >
                  ...
                  <div className="bg-card rounded-lg border border-border overflow-hidden">
                    <div className="relative aspect-[2/3]">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                        <Badge className="text-xs mb-1 bg-purple-500/80">
                          Novel
                        </Badge>
                        <h3 className="text-white font-bold text-sm line-clamp-2">
                          {formData.title || 'Título da Novel'}
                        </h3>
                      </div>
                    </div>
                    <div className="p-3 space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground line-clamp-3">
                        {formData.synopsis || 'Sinopse da novel aparecerá aqui...'}
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Dados principais da novel</CardDescription>
                </CardHeader>
                ...
                <CardContent>
                  ...
                  {formData.genres.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-border">
                      <p className="text-sm text-muted-foreground mb-2">Selecionados ({formData.genres.length}):</p>
                      <div className="flex flex-wrap gap-2">
                        {formData.genres.map((genre) => (
                          <Badge key={genre} className="bg-purple-500">
                            {genre}
                            <button
                              type="button"
              ...
              <Card className="border-purple-500/30 bg-purple-500/5">
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground">
                    Após criar a novel, você poderá adicionar capítulos com texto na página de 
                    <strong> Upload de Capítulo</strong>. Selecione "Novel (Texto)" como tipo de conteúdo 
                    para escrever capítulos em formato de texto.
                  </p>
                </CardContent>
              </Card>
